...
* **Wikilink Scope Limits**: Never use Obsidian wikilinks `[[filename]]` for files located outside the `/Users/uynx/ai_memory/` vault. For external files (such as files in `~/nix-config/` or local configurations), use standard relative paths or inline code formatting.
